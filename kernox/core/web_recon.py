...
Given a web address, runs a full recon chain:
  ...
  6. Directory fuzzing (ffuf) — asks
  7. Subdomain enum (dnsrecon) — asks
  8. CMS detection → wpscan if WordPress found
"""
...
class WebReconChain:
    ...
    def run(self, target: str) -> list[dict]:
            ...
            if r:
                ...
                if "wordpress" in techs or "wp" in techs:
                    ...
                    if self._ask_step("3b", "WordPress deep scan", "wpscan"):
                        wr = self._orc._run_tool("wpscan", {"target": target, "mode": "full"})
                        if wr:
        ...
        # Step 6 — Directory fuzzing (optional)
        ...
        # Step 7 — Subdomain enumeration (optional)
        ...
        # Step 8 — Nuclei vulnerability scan (optional)
